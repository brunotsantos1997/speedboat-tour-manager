              ...
              <div className="flex gap-4">
                <label className="flex items-center gap-2 cursor-pointer p-3 border rounded-lg hover:bg-gray-50 flex-1">
                  <input
                    ...
                    value="RENTAL_ONLY"
                    checked={formData.commissionBasis === 'RENTAL_ONLY'}
                    onChange={() => setFormData(prev => prev ? { ...prev, commissionBasis: 'RENTAL_ONLY' } : null)}
                    className="h-4 w-4 text-blue-600 focus:ring-blue-500"
                  />
                  <span className="text-gray-700">Apenas Aluguel do Barco</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer p-3 border rounded-lg hover:bg-gray-50 flex-1">
                  <input
                    type="radio"
                    name="commissionBasis"
                    value="TOTAL_PRICE"
                    checked={formData.commissionBasis === 'TOTAL_PRICE'}
                    onChange={() => setFormData(prev => prev ? { ...prev, commissionBasis: 'TOTAL_PRICE' } : null)}
                    className="h-4 w-4 text-blue-600 focus:ring-blue-500"
                  />
                  <span className="text-gray-700">Total do Passeio (Barco + Extras)</span>
                </label>
              </div>
            </div>
          </div>
